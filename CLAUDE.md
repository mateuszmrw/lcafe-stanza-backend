...
ARQ worker — tokenize_page, align_smil_audio, generate_tts_audio, import_youtube_subtitles
    ↓ Stanza NLP / OpenAI-compatible TTS / yt-dlp / trafilatura
...
- OpenAI — grammar explanation and synonym nuance (LLM cascade: DB system key → env var)
- OpenAI-compatible TTS — text-to-speech generation for books without embedded audio (any server implementing `POST /v1/audio/speech`)
...
| `MAX_UPLOAD_BYTES`            | `104857600` (100 MB) | Generic upload cap (SRT subtitles, frequency CSVs)           |
| `MAX_BOOK_UPLOAD_BYTES`       | `524288000` (500 MB) | EPUB/PDF upload size limit                                   |
| `MAX_DICTIONARY_UPLOAD_BYTES` | `2147483648` (2 GB)  | Dictionary dump upload size limit (admin)                    |
...
| `OPENAI_TTS_URL`              | —                    | OpenAI-compatible TTS server base URL (optional)             |
| `OPENAI_TTS_API_KEY`          | —                    | Bearer token for the TTS server (optional)                   |
| `OPENAI_TTS_MODEL`            | `qwen3-tts`          | Model id sent to `/v1/audio/speech`                          |
